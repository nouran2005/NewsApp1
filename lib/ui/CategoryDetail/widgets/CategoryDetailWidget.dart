...
class Categorydetailwidget extends StatefulWidget {
  const Categorydetailwidget({super.key, required this.categoryID,this.searchQuery});
                ...
                Expanded(
                  child: TabBarView(
                    children: response?.sources
                            ?.map((Source) => newsListWidget(sourceID: Source.id ?? "" , searchQuery: widget.searchQuery,))
                            .toList() ??
                        [],
                  ),
                ),
...
class _CategorydetailwidgetState extends State<Categorydetailwidget> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      key:  ValueKey(widget.searchQuery),  // Ensures the widget rebuilds on query changes
      future: Apimaneger.getSources(widget.categoryID),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Center(
